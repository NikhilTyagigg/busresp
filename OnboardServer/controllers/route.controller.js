...
const { NotAuthorizedError, ApplicationError, UserError, ConflictError, NotFoundError, ForbiddenError, InternalError } = require('../helper/errors');
...
main.configRoutes = async() =>{
    try{
        let sampleData = {"OnBoard":"DL9CAB0975","Bus Module ID": 753,"Route No": "1243","source":'web app'}
...
main.addVehicle = async(data) =>{ //this will be used for adding/updating and deleting a vehicle
    ...
    try{
        ...
        if(data.vehicleId){
            ...
            if(data.vehicleNo != vehicle.vehicleNo.trim().toUpperCase()){
            ...
            else if(data.vehicleModule != vehicle.vehicleModule.trim().toUpperCase()){
                let oldVehicle = await Vehicle.findOne({
                    where : {
                ...
                if(oldVehicle && oldVehicle.isActive){
                    {
                        throw new ConflictError("Module is already mapped to vehicle No:: "+ oldVehicle.vehicleNo)
                    }
                }else if(oldVehicle){
                    await vehicle.update({
                        isActive : false
            ...
            vehicle = await Vehicle.findOne({
                where : {
                    [Op.or] : {vehicleNo : data.vehicleNo,
                        vehicleModule : data.vehicleModule
                     }
                }
            })
            ...
            if(vehicle && vehicle.isActive){
                if(data.vehicleModule == vehicle.vehicleModule){
                    throw new ConflictError("Module is already mapped to vehicle No:: "+ vehicle.vehicleNo)
                }
...
main.addRoute = async(data) =>{ //this will be used for adding/updating and deleting a vehicle
    logger.info('Adding/updating route details::' + JSON.stringify(data) )
    try{
        let route = null
        if(!data.routeNo || !data.startPoint || !data.endPoint){
            throw new UserError('Please enter all the valid parameters!!')
        }
...
main.addVehicleRouteMap = async(data) =>{ //this will be used for adding/updating and deleting a vehicle
    logger.info('Adding/updating route details::' + JSON.stringify(data) )
    try{
        let route = null
...
main.addMultipleRoute = async(data=[]) =>{ //this will be used for adding/updating and deleting a vehicle
    logger.info('Adding/updating route details::' + JSON.stringify(data) )
    try{
        ...
        let routesData =[]
         data && data.map(d =>{
            if(!d[0] || d.length<3){
                throw new ConflictError('Invalid entry present in the sheet, please check and upload again!!')
            }
            if(d[0].length>6){
                throw new ConflictError('Route Number cannot be greater than 6 digits, please check and upload again!!')
            }

            let regex = new RegExp("^[a-zA-Z0-9_]+$");
            if(!regex.test(d[0])){
                throw new ConflictError('Invalid route no' + d[0] + 'Only alphanumeric route number data is allowed, please check and upload again!!')
            }
            let rNo = d[0]
            rNo = rNo.toString()
            while(rNo.length<6){
        ...
        if(isDuplicate){
            logger.error('Duplicate entries for same route number exists. Please maintain single entry for each route No.')
            throw new ConflictError('Duplicate entries for same route number exists. Please maintain single entry for each route No.')
        }
        await Route.bulkCreate(routesData,{
            updateOnDuplicate: ['startPoint', 'endPoint', 'intermediateStops', 'isActive']
        })
...
main.addMultipleVehicles = async(data=[]) =>{ //this will be used for adding/updating and deleting a vehicle
    logger.info('Adding/updating vehicle details details::' + JSON.stringify(data) )
    try{
        if(!data || data.length==0){
        ...
        if(isDuplicate){
            logger.error('Duplicate entries for same vehicle number exists. Please maintain single entry for each vehicle No.')
            throw new ConflictError('Duplicate entries for same vehicle number exists. Please maintain single entry for each vehicle No.')
        }
